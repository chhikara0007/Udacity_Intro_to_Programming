...
'''Added this hypothetical SQL proc to reduce redundancy.  Is this the best way
to reduce the repetition in the functions below?'''
...
def registerPlayer(name, tournament_id):
	...
	scoreboard = """INSERT INTO scoreboard (tournament_id, player, score, matches, bye)
					VALUES (%s, %s, %s, %s, %s)"""
	param = (name,)
...
def playerStandings(tournament_id):
	...
	players = """SELECT s.player, p.name, s.score, s.matches, s.bye,
				...
				OR s2.player IN (SELECT winner
								...
								AND tournament_id = %s)) as opponent_strength
...
def reportMatch(winner, loser):
	...
	DB = connect()
	c = DB.cursor()
	ins = "INSERT INTO matches (tournament_id, winner, loser, draw) VALUES (%s,%s,%s,%s)"
	win = "UPDATE scoreboard SET score = score+%s, matches = matches+1 WHERE player = %s AND tournament_id = %s"
	los = "UPDATE scoreboard SET score = score+%s, matches = matches+1 WHERE player = %s AND tournament_id = %s"
...
def reportBye(player, tournament_id):
	...
	DB.connect()
	c.DB.cursor()
	bye = "UPDATE scoreboard SET score = score+3, bye = bye+1, WHERE player = %s AND tournament_id = %s"
